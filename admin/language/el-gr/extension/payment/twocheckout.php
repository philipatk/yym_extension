<?php
// Heading
$_['heading_title']                 = '2Checkout';

// Text
$_['text_extension']                = 'Επεκτάσεις';
$_['text_success']                  = 'Επιτυχία: Έχετε επεξεργαστεί τις πληροφορίες λογαριασμού του 2Checkout!';
$_['text_edit']                     = 'Edit 2Checkout';
$_['text_twocheckout']              = '<a href="https://www.2checkout.com/2co/affiliate?affiliate=1596408" target="_blank"><img src="view/image/payment/2checkout.png" alt="2Checkout" title="2Checkout" style="border: 1px solid #EEEEEE;" /></a>';

// Entry
$_['entry_account']                 = '2Checkout ταυτότητα λογαριασμού (Account ID)';
$_['entry_secret']                  = 'Μυστική λέξη (Secret Word)';
$_['entry_display']                 = 'Direct Checkout';
$_['entry_test']                    = 'Δοκιμαστική Λειτουργία';
$_['entry_total']                   = 'Σύνολο';
$_['entry_order_status']            = 'Κατάσταση Παραγγελίας';
$_['entry_geo_zone']                = 'Γεωγραφική Ζώνη';
$_['entry_status']                  = 'Κατάσταση';
$_['entry_sort_order']              = 'Σειρά Ταξινόμησης';

// Help
$_['help_secret']                   = 'Η μυστική λέξη για την επιβεβαίωση συναλλαγών  (πρέπει να είναι η ίδια, όπως ορίστηκε στη σελίδα ρυθμίσεων λογαριασμού εμπόρου - merchant account).';
$_['help_total']                    = 'Το συνολικό ποσό που πρέπει να φτάσει η παραγγελία για να ενεργοποιηθεί η μέθοδος πληρωμής.';

// Error
$_['error_permission']              = 'Ειδοποίηση: Δεν έχετε άδεια να επεξεργαστείτε την πληρωμή 2Checkout!';
$_['error_account']                 = 'Απαιτείται η ταυτότητα λογαριασμού (Account ID)!';
$_['error_secret']                  = 'Απαιτείται η Μυστική λέξη (Secret Word)!';